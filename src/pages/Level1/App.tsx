    ...
    return (
      <div style={cardStyle}>
        ...
        <button onClick={handleSign} style={{
          width: '100%', padding: '9px 0', borderRadius: 6, background: colors.cyan,
          border: 'none', color: colors.bgBase,
          fontFamily: fonts.display, fontSize: 10, cursor: 'pointer', fontWeight: 700,
        }}>
          Sign for Delivery
        </button>
      </div>
    ...
    <div style={cardStyle}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={handleLetIn} style={{
          ...
          border: `1px solid ${colors.red}`, color: '#ff6688',
          fontFamily: fonts.display, fontSize: 9, cursor: 'pointer',
        }}>
          Sure, come in →
        </button>
  ...
  return (
    <>
      ...
      {sweepComplete && ATTACK_POINTS.map((a, index) => {
        ...
        return <AnnotationPoint key={a.id} id={a.id} displayName={a.displayName} position={pos} />
      })}
    </>
  )
}
...
function OfficeSweep({ active, onComplete }: OfficeSweepProps) {
  ...
  const SWEEP_DURATION = 7.0                              // total seconds for full sweep (slower)
  ...
  const START_ANGLE    = Math.PI * 0.8                    // start angle (radians)
...
// ─── Root ─────────────────────────────────────────────────────────────────────
export default function Level1Simulation({ onExit }: Level1SimulationProps) {
