...
export default function MessagesApp() {
  ...
  const handleVerify = () => {
    ...
    setConsequence(
      'You visited a credential-harvesting site. The URL "hsbc-secure-verify.net" is not HSBC\'s website (hsbc.co.uk). Attackers register similar-sounding domains.\n\nThe sender name "HSBC ALERT" can be spoofed by anyone — it does not verify origin.\n\nAlways contact your bank using the number on the BACK of your card.'
    )
  }
  ...
  const handleWrongChoice = (reason: string) => {
    ...
    setConsequence('Credential theft risk: this link opens a fake banking portal designed to steal login codes.')
  }
        ...
        <div onClick={() => setDetail(true)}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
              <div style={{ ...mono, fontSize: 13, color: '#ff6688', fontWeight: 700 }}>HSBC ALERT</div>
  ...
  return (
    <div style={{ position: 'relative', height: '100%', background: '#06080f', color: '#e5e7eb', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '12px 16px', fontFamily: "'Orbitron',sans-serif", fontSize: 12, color: '#00f0ff', borderBottom: '1px solid #1a1f2a', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        ...
        <span style={{ color: '#ff6688' }}>HSBC ALERT</span>
        <span style={{ ...mono, fontSize: 9, color: '#3a3f4a', marginLeft: 4 }}>Sender ID — spoofable</span>
      </div>
      ...
      <div style={{ flex: 1, padding: '16px 14px', overflowY: 'auto' }}>
        ...
        <div style={{ textAlign: 'center', ...mono, fontSize: 10, color: '#3a3f4a', marginBottom: 16 }}>
          Sender name can be set by anyone · Not verified
        </div>
        ...
        <div style={{ background: '#0d1117', borderRadius: '14px 14px 14px 2px', padding: '12px 14px', maxWidth: '88%', marginBottom: 12 }}>
          <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#9ca3af', lineHeight: 1.65 }}>
            HSBC ALERT: A transaction of £2,847.00 was attempted from your account.
            <br /><br />
            ...
            <span
              ...
              style={{ color: '#4488ff', textDecoration: 'underline', cursor: 'help', ...mono, fontSize: 12 }}>
              hsbc-secure-verify.net
            </span>
            <br /><br />
          ...
          {hoverUrl && (
            <div style={{ marginTop: 8, ...mono, fontSize: 10, color: '#ff6644', background: '#1a0808', padding: '3px 8px', borderRadius: 3 }}>
              ⚠ Not hsbc.co.uk — suspicious domain
            </div>
          )}
        </div>
        ...
        {false && succeeded && (
          <div style={{ padding: '12px 14px', border: '1px solid #00ff88', borderRadius: 8, ...mono, fontSize: 12, color: '#00ff88', lineHeight: 1.7 }}>
            ...
            <span style={{ fontSize: 11, color: '#00cc66' }}>
              The sender name can be spoofed. The URL reveals the truth —<br />
              it's not hsbc.co.uk. Never click links in bank texts.
            </span>
          </div>
        )}
        ...
        {succeeded && (
          <div style={{ padding: '12px 14px', border: '1px solid #00ff88', borderRadius: 8, ...mono, fontSize: 12, color: '#00ff88', lineHeight: 1.7 }}>
            Correct! +120 pts<br />
            <span style={{ fontSize: 11, color: '#00cc66' }}>
              Threat neutralized. The clue was the unofficial banking domain and urgent fraud language. Always verify through an official channel.
            </span>
          </div>
        )}
      </div>
      ...
      {false && !succeeded && !consequence && (
        <div style={{ padding: '12px 14px', borderTop: '1px solid #1a1f2a', display: 'flex', flexDirection: 'column', gap: 10, flexShrink: 0 }}>
          <button onClick={handleVerify} style={{
            padding: '11px', borderRadius: 8, background: 'rgba(255,51,85,0.12)',
            border: '1px solid #ff3355', color: '#ff6688',
            fontFamily: "'Orbitron',sans-serif", fontSize: 10, cursor: 'pointer', letterSpacing: '0.06em',
          }}>
            ⚡ VERIFY NOW — Freeze Transaction
          </button>
          <button onClick={handleIgnore} style={{
            padding: '11px', borderRadius: 8, background: '#001a0f',
      ...
      {!succeeded && !consequence && (
        <div style={{ padding: '12px 14px', borderTop: '1px solid #1a1f2a', display: 'flex', flexDirection: 'column', gap: 10, flexShrink: 0 }}>
          <button onClick={() => handleWrongChoice('tapped_sms_link')} style={{
            padding: '11px', borderRadius: 8, background: 'rgba(255,51,85,0.12)',
            border: '1px solid #ff3355', color: '#ff6688',
            fontFamily: "'Orbitron',sans-serif", fontSize: 10, cursor: 'pointer', letterSpacing: '0.06em',
          }}>
          ...
          <button onClick={() => handleWrongChoice('replied_with_account_details')} style={{
            padding: '11px', borderRadius: 8, background: 'rgba(255,51,85,0.12)',
            border: '1px solid #ff3355', color: '#ff6688',
            fontFamily: "'Orbitron',sans-serif", fontSize: 10, cursor: 'pointer', letterSpacing: '0.06em',
          }}>
          ...
          <button onClick={handleCorrectChoice} style={{
            padding: '11px', borderRadius: 8, background: '#001a0f',
            border: '1px solid #00ff88', color: '#00ff88',
            fontFamily: "'Orbitron',sans-serif", fontSize: 10, cursor: 'pointer',
          }}>
